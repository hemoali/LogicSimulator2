...
bool Utils::CheckPoint(GraphicsInfo r_GfxInfo, CellType usedPixels[44][74]){
	...
	if ((r_GfxInfo.x1 - UI.GATE_Width / 2.0)<=0 || (r_GfxInfo.y1 - UI.GATE_Height / 2.0) <= UI.ToolBarHeight || (r_GfxInfo.x1 + UI.GATE_Width / 2.0)>=UI.width-16 || (r_GfxInfo.y1 + UI.GATE_Height / 2.0) >= UI.height - UI.StatusBarHeight)return 0;
