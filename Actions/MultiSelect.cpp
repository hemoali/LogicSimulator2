...
void MultiSelect::Execute()
{
	...
	if (pManager->GetInput()->getComponentToBeAddedToSelected() != NULL) // check if new comp to be added for the selected components
	{
		...
		{
			if (pManager->getComponent(i) == pManager->GetInput()->getComponentToBeAddedToSelected())
			{
				allSelectedComponents.push_back(make_pair(i, pManager->GetInput()->getComponentToBeAddedToSelected()));
				break;
			}
		}
